...
async function githubCommand(sock, chatId, message) {
  try {
    const res = await fetch('https://api.github.com/repos/tkttech/TKT-CYBER-TECH');
    if (!res.ok) throw new Error('Error fetching repository data');
    const json = await res.json();
    


    let txt = `🚀 *TKT-CYBER-TECH REPO INFO* 🚀
...
┊⭘ ʙᴏᴛ ʟɪɴᴋ: https://github.com/TKTTECH1/TKT-CYBER-TECH
╰────────━⊷
    `;
